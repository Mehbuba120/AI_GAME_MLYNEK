import pygame
from openPhase3 import *
from movingPhase3 import *
...
# mixer.init()
# mixer.music.load('static/song.mp3')
# mixer.music.set_volume(1.0)
# mixer.music.stop()

#images
boardImg = pygame.image.load('static/board.jpg')
	...
	if turn > 18:
		...
		for b in board:
			...
			if b == 'W':
				cnt2 += 1
		if cnt2 < 3:
			gameComplete = -1    # for W 
		if cnt1 < 3:
			gameComplete = 1     #for B

		Game = MiniMaxGame(3)

		if Game.GenerateMovesMidgameEndgame(board, True) == []: #for 'B' no moves left
			gameComplete = 1
		if Game.GenerateMovesMidgameEndgame(board) == []:       #for 'W' 
			gameComplete = -1

#for human
#played=true human done; AI start 
#selectmove=false; AI move
def drawBoard():
...
def drawText():
	...
	if gameComplete == 1:                          #B lost, human
		screen.blit(overRobo1, (50, 550))
		# mixer.music.play()
	if gameComplete == -1:                         #w lost, AI
		screen.blit(overRobo1, (50, 550))
		# mixer.music.play()
